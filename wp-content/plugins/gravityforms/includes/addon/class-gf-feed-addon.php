...
abstract class GFFeedAddOn extends GFAddOn {
	...
	public function get_feeds( $form_id = null ) {
		...
		$feed_order = $this->get_feed_order( $form_id );
		if ( $results && $feed_order ) {
			
			usort( $results, function( $a, $b ) use ( $feed_order ) {
				$position_a = array_search( $a['id'], $feed_order );
				$position_b = array_search( $b['id'], $feed_order );
	...
	public function get_feed_order( $form_id ) {
		
		/* If feed ordering is not suported, return null. */
		...
		/* Return the feed order for this Add-On. */
		return rgars( $form, 'feedOrder/' . $this->_slug );
	...
	public function ajax_save_feed_order() {
		...
		$feed_order = rgpost( 'feed_order' );
		
		if ( $addon == $this->_slug ) {
			$this->save_feed_order( $form_id, $feed_order );
		}
	}
